...
\TLV
   ...
   //program counter
   //PC starts at 0 on each reset and increments by 4
   //$pc[31:0] = $reset ? 0 : (>>1$pc + 4);
   //$pc[31:0] = $reset ? 0 : ($next_pc);
   $pc[31:0] = >>1$next_pc;
   ...
   //checks isntruction type based on opcode field bits
   ...
   //extract fields from isntruction
   $rs2[4:0] = $instr[24:20];
   $rs1[4:0] = $instr[19:15];
   ...
   //immediate decoding based off type
   ...
   $is_aupic = $dec_bits ==? 11'bx_xxx_0010111;
   ...
   $slru_result[31:0] = {31'b0, $src1_value < $src2_value};
